# Portfolio Shared Server Specification

This project can run without the shared backend, but a few online/shared features will stay offline until a separate server is running.
...
- `api/multiplayer.js`: room presence / ghost car state.
- `VITE_SERVER_URL` WebSocket server: whispers, scores, shared counters, cataclysm, easter data.
